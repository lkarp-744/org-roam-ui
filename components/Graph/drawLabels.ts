...
export interface drawLabelsProps {
  ...
  ctx: any;
  globalScale: number;
  highlightedNodes: { [id: string]: object };
  previouslyHighlightedNodes: { [id: string]: object };
...
export function drawLabels(props: drawLabelsProps) {
  ...
  const fadeFactor = Math.min(
    5 * (globalScale - visuals.labelScale) +
      2 *
        Math.pow(
          Math.min(links.length, visuals.labelDynamicDegree),
          visuals.labelDynamicStrength
        ),
    1
  );
  ...
  const highlightedNodeOffset = [hoverId, lastHoverId].includes(
    (node as OrgRoamNode).id
  )
    ? 1 + 0.3 * opacity
    : 1;
